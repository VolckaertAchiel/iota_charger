    ...
    socket.on('history', ()=>{
        iota.api.getTransfers(seed ,(error, success) => {
            if (error) {
                ...
                console.log("success")
                socket.emit('message', { value:success});
            }
        })
        socket.emit('seedLoaded', {value: 'valid'});
    })
    ...
    setInterval(() => {
        tempSensor.read((err, reading) => {
            ...
            if(amps > 0.25){
                ...
                ChargeUsage(seed,"you charged an average off: " + averageWatt + "W , in : " + totalSeconds+" Seconds and costs: "+ elecPrice )
...
function ChargeUsage(SeedSeed, message){
